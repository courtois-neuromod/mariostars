...
| **Outcome** | `cleared` (flag hit[state 4] & lives≥0), `failed/timeout` (timer=0), `failed/fall` (life lost, no state 11), `failed/killed` (life lost, state 11), `incomplete/warp` (no flag/death, `current_world`/`current_level` changed = warp-zone pipe exit e.g. W1-2/W4-2), `incomplete/interrupted` (no flag/death/warp = recording cut mid-level). |
