"""Event-file tests for the mariostars dataset.
...
def test_sidecar_present_and_complete():
    ...
    sidecar = op.join(DATAPATH, f"task-mariostars_events.json")
...
def test_every_emitted_type_is_allowed_for_this_task():
    ...
    assert not wrong, f"trial_types not allowed for mariostars: {wrong[:10]}"
